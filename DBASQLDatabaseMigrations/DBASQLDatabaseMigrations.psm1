...
.DESCRIPTION 
	Execute a sql query, return the results as a table
.EXAMPLE 
	Execute-SQLQuery -InstanceName "AServer" -DatabaseName "DBAdmin" -query "SELECT * FROM dbo.MyTable" -ApplicationName "MyTestApplication"
...
.EXAMPLE 
	Test-AlwaysOnEnabled -InstanceName "ABlueDellServer"
.PARAMETER InstanceName 
	Instance of SQL server to query.
.OUTPUT
...
.EXAMPLE 
	Backup-MultiFileSQLDatabase -InstanceName "SomeServer" -DatabaseName "Master" -BackupPath "\\BackupServerSharePath\SQL_Backup2\Migrations"
.PARAMETER InstanceName 
	Instance of SQL server to query.
.PARAMETER DatabaseName
...
.DESCRIPTION 
	Changes the recovery model of a database using Invoke-SQLCmd and a written query statement instead of SQL ps objects.  
.EXAMPLE 
	Set-DatabaseRecoveryModel -InstanceName TestServer -DatabaseName "Model" -RecoveryModel "FULL"
.PARAMETER InstanceName 
	Instance of SQL server to query.
.PARAMETER DatabaseName
    Name of the database to be backed up
