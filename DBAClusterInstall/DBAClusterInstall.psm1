#Is the bulk of setting up a cluster, calling the smaller functions
##TODO Add flag for storage vs no storage
## add node preference for IPs so subnet1 ips only go to subnet1 nodes etc.
...
.DESCRIPTION 
	...
	the cluster instead.
.EXAMPLE 
	Initialize-NewCluster -HostName MyFavoriteComputer -ClusterName "MyNewCluster" -ClusterIPAddress "172.19.23.15" -SQLOU OU=SQL.OU=Computers.DC=place.DC=aa.=DC=com -fileShareWitnessPath \\SomeServer\SomeShare -DataCenter Primary -ListenerGroup SQL_AOLN_Creators
.PARAMETER HostName
...
#>
   ...
   Try
   {
		...
		if(Test-ADComputerObjectExists -ObjectDisplayName $ClusterName)
		{
			...
			{
				Set-ClusterGroupIPDependency -ClusterName $ClusterName -ClusterGroupName "Cluster Group" -ClusterResourceName "Cluster Name" -ClusterNameIPResourceName1 ($ClusterIPName.Replace("DR","PD")) -ClusterNameIPResourceName2 ($ClusterIPName.Replace("PD","DR"))
			}
			...
			{
				Add-ClusterGroupResourceIP -ClusterName $ClusterName -ClusterGroupName "Cluster Group" -ClusterIPName $ClusterIPName -IpAddress $ClusterIpAddress
				$ipResources = Get-ClusterResource -Cluster $ClusterName | WHERE { $_.ResourceType -eq "IP Address" -and $_.OwnerGroup -eq "Cluster Group"}
				if($ipResources.Count -eq 2)
				{
					Set-ClusterGroupIPDependency -ClusterName $ClusterName -ClusterGroupName "Cluster Group" -ClusterResourceName "Cluster Name" -ClusterNameIPResourceName1 ($ClusterIPName.Replace("DR","PD")) -ClusterNameIPResourceName2 ($ClusterIPName.Replace("PD","DR"))
	   ...
	   #Cluster does not exist in AD
		else
		{
			...
			$CNPath = "CN=$ClusterName,"+$SQLOU+"DC=my,DC=domain,DC=com"
...
.OUTPUTS 
	Bool
.EXAMPLE 
	Resolve-IsNodeMemberOfCluster -HostName "SOMEServer" -ClusterName "TestCluster"
.EXAMPLE
	Resolve-IsNodeMemberOfCluster -HostName SOMEServer -ClusterObj $MyClusterObject
#>
	[cmdletbinding()]
	param
	(
...
Function Add-NewClusterNode
{
    ...
    (
        ...
        {
				Add-ClusterGroupResourceIP -ClusterName $clusterName -ClusterIPName -ClusterGroupName "Cluster Group" $ClusterIpAddressName -IpAddress $ClusterIpAddress 
        }
}
...
.EXAMPLE 
	Set-ClusterPermissions -Identity "DOMAIN\DBAdmins" -Grant -PermissionType FULL -ClusterName "MyCluster"
	Grants full access to DBAdmins group on MyCluster
.EXAMPLE 
	Set-ClusterPermissions -Identity "DOMAIN\DBAdmins" -Grant -PermissionType FULL -ClusterObj $ClusterObj
	Grants full access to DBAdmins group on the ClusterObject (object type Cluster).
.EXAMPLE 
	Set-ClusterPermissions -Identity "DOMAIN\DBadmins" -ClusterName "MyCluster"
...
Function Add-ClusterGroupResourceIP
{
    [cmdletbinding()]
    param
    (
        [Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the cluster name?')]
        [string] $ClusterName,
        [Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the cluster group/role this IP will belong to?')]
    ...
    try
    {
        Write-Log -Level INFO -Message "Checking if Cluster IP needs to be added/updated to Cluster Group $clusterGroupName as IP Resource.  Cluster: $ClusterName, Network Resource Name: $ClusterIPName, IP: $IpAddress"
		...
		{
			Write-Log -Level ERROR -Message "Cannot add NULL ip to Cluster: $ClusterName, for Cluster GrouP: $clusterGroup for Cluster IP named: $ClusterIPName"
		}
		...
		Else
		{
			Write-Log -Level INFO -Message "IP address: $IpAddres is already a resource of the cluster $ClusterName.  Skipping addition of IP Resource."
		}
    }
    catch
    {
...
function Set-ClusterGroupIPDependency
{
	<# 
.SYNOPSIS 
	Set Dependency for Group resource on IPs.
.DESCRIPTION 
	Set Dependency for Group resource on IPs. Sets the dependency with the OR condition
.EXAMPLE 
	Set-ClusterGroupIPDependency -ClusterName "MyCluster" -ClusterGroupName "Cluster Group" -ClusterResourceName "Cluster Name" -ClusterNameIPResourceName1 "CLuster IP PD" -ClusterNameIPResourceName2 "Cluster IP DR"
.PARAMETER ClusterObj
	Microsoft Failover Clusters Powershell Cluster object representing the cluster to be updated
.PARAMETER HostRecordName
	ARecord to be udpated.
...
#>
	...
	(
		[Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the cluster name?')]
        [string] $ClusterName,
        [Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the cluster group/role we are modifying?')]
        [string] $ClusterGroupName,
        [Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the cluster resource name?')]
        [string] $ClusterResourceName,
		[Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='what is first the cluster IP resource name?')]
        [String] $ClusterNameIPResourceName1,
		[Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='what is second the cluster IP resource name?')]
        [String] $ClusterNameIPResourceName2
	)

    #Validate cluster resource
    $cluster = Get-Cluster -Name $ClusterName
    #limit our needs down to the our cluster group
    $clusterGroup = $cluster|get-clusterGroup -Name $ClusterGroupName
    #get our resource
    $ResourceObj = $clusterGroup | Get-ClusterResource -Name $ClusterResourceName
	#get dependencies for Cluster Resource
    if ($ResourceObj.ResourceType.Name -ne "Network Name")
    {
        Write-Log -Level ERROR -Message "The Cluster Resource ($ClusterResourceName) is not of type Network Name.... You cannot set IPs as dependencies for Non Network Names!!!!"
    }
    if($ResourceObj -ne $null)
    {
	    $DependencyExpression = (Get-ClusterResourceDependency -InputObject $ResourceObj -ErrorAction STOP).DependencyExpression
    }
    ELSE
    {
         Write-Log -Level ERROR -Message "Unable to find Cluster Resource ($ClusterResourceName) in Cluster Group ($clusterGroupName) on cluster ($ClusterName)"
        Throw "failure"
    }
	...
	{
		Write-Log -Level INFO -Message "Setting resource dependencies for Cluster ($ClusterName) in cluster group ($clusterGroupName) for ClusterResourceName($ClusterResourceName)"
		try
		{
			Set-ClusterResourceDependency -InputObject $ResourceObj -Dependency "[$ClusterNameIPResourceName1] or [$ClusterNameIPResourceName2]" -ErrorAction STOP
		}
		catch
		{
			$ExceptionMessage = $_.Exception.Message
			Write-Log -Level ERROR -Message "Issue Setting Cluster Name Resource Dependencies on cluster $ClusterName. $ExceptionMessage"
		}
		Write-Log -Level INFO -Message "Cluster Network Name Resource Dependencies updated on Network Name: $ClusterResourceName to [$ClusterNameIPResourceName1] or [$ClusterNameIPResourceName2]  For Cluster: $ClusterName"
	}
}
