#
# DBASQLServerInstall.psm1
function Confirm-IsSQLAlreadyInstalled
{
<# 
  .SYNOPSIS 
  Checks if the default instance of sql is already installed
  .DESCRIPTION 
  Checks for default instance via service list.
  .EXAMPLE 
  New-SQLFolders -HostName "TestSQLServer"
  .PARAMETER HostName
  The Name of host to check for a default instance.
  .OUTPUT
  returns true if default instance is found
#>
[cmdletbinding()]
    param
    (
...
<# 
  .SYNOPSIS 
  Creates base folders for standard sql server installs on the approriate disks.
  .DESCRIPTION 
...
#> 
    ...
    (
        ...
        [ValidateSet("SQL","SSIS","SSRS","FailOverClusterNode")]
        [String]$Engine = "SQL"
    )
...
function Copy-SQLInstallFiles
{
    ...
    (
        ...
        [string] $SQLPatches,
		[Parameter(Mandatory=$false,Position=6,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='Path to dump SQL ISO')]
        [string] $SQLIsoDestinationDirectory
    )
    ...
    try
    {
        ...
        $Destination = "$LocalDriveLetter$\"
		
		if($SQLIsoDestinationDirectory -ne $null)
		{
			$Destination += $SQLIsoDestinationDirectory
		}
		...
		try
		{
			...
			$SourceLoc = (($mountResult|Get-Volume).DriveLetter +":\")
		}
		catch
		{
			$ExecptionMessage = $_.Exception.Message
		...
		$UpdateDirectory = "\\$HostName\$Destination" + "Updates"
		...
		If(($InstallationMediaPathPatches.Length -gt 1))
        {
			...
			{
				Write-Log -Level INFO -Message "Copying SQL $SQLVersion PATCH Media from: ($PatchPath)  to: ($UpdateDirectory)"
				Copy-DirectoryWithBITSTransfer -SourceDirectory  $PatchPath -DestinationDirectory "$UpdateDirectory"
				Write-Log -Level INFO -Message "Copy of PATCHES from: ($PatchPath) to: ($UpdateDirectory) complete"
			}
        }
        ...
        $ConfigFileDestination = "\\$Hostname\$Destination"
		Start-BitsTransfer -Source $ConfigurationIniFile -Destination "$($ConfigFileDestination)SQLConfiguration.ini" -Description "Transfer of SQL Configuration.ini file" -DisplayName "SQLConfiguration.ini"
		Write-Log -Level INFO -Message "Copy SQLConfiguration.ini file to: ($ConfigFileDestination) complete"
		return $SourceLoc;
    }
    catch
    {
