...
function Get-DatabasesReadyForMirroring
{
    Param
    (
    	[Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='What is the Primary instance name?')]
		[string]$PrimaryInstanceName,
        [Parameter(Mandatory=$true,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true,HelpMessage='Comma Separate lists of DBs')]
		[string[]]$DatabaseList

    )

    [string] $whereClauseDbList

    if($DatabaseList.Count -gt 0)
    {
        [string] $test = "'$($DatabaseList -join "','")'"
        $whereClauseDbList = "AND d.name IN ($test )"
    }
$Query = @"
...
"@
 ...
 $Results = Invoke-Sqlcmd -ServerInstance $PrimaryInstanceName -Query $Query 
 return $Results
...
{
	$BackupFilePath ="\\BackupServer\Backups\MirrorSetup\"
} 
...
Foreach($DB in $Databases)
{
  ...
  $PrincipalEndpoint = 'TCP://'+$PrincipalInstance+'.'+$DomainName+':5022'
  ...
  $FailoverEndpoint = 'TCP://'+$FailoverPartnerInstance+'.'+$DomainName+':5022'
