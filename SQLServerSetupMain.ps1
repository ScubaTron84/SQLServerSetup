...
function SetOSSettings
{
	...
	IF ($DistinguishedHostName -eq "CN=$Global:HostName,OU=SQL,OU=Computer Resources,DC=your,DC=domain,DC=com")
	{
		Write-Log -Level INFO -Message "OU Path for host: $Global:HostName is correct.  OU Path: $DistinguishedHostName"
	}
	Else
	{
		Write-Log -Level INFO -Message "OU Path for host: $Global:HostName is incorrect.  Moving from OU Path: $DistinguishedHostName to correct OU Path: CN=$Global:HostName,OU=SQL,OU=Computer Resources,DC=your,DC=domain,DC=com"
		$DestinationOU = "CN=$Global:HostName,OU=SQL,OU=Computer Resources,DC=your,DC=domain,DC=com"
		try
		{
...
#Install SQL Server Remotely
	If([System.Convert]::ToBoolean($Global:RunInstallSQLRemotely))
	{
		Install-SQLServerRemotely -HostName $Global:HostName -SQLServiceAccount $Global:SQLServiceLogin -Password $Global:SQLServiceLoginPassword `
			...
			-SQLTempDBDirectory	"$($Global:TempDBDisk):$($Global:TempDBPath)"` -SQLSysAdminAccounts "Domain\DBAdmins" -SQLVersion $Global:SQLVersion
